...
public class VirtualChestHandler {
	...
	private Inventory createChest(String playerName) {
		Inventory chest = Bukkit.createInventory(null, 54, SeCon.getAPI().getChatUtils().formatMessage(pc.getConf().getString("virtualchest-name")));
	...
	public String getTableName() {
		try {
			return SeCon.getAPI().getDatabaseUtils().getDefault().tableName("virtualchests");
	...
	public void load(String playerName) {
		try {
			...
			if (serialized != null) {
				HashMap<Integer, ItemStackInfo> tmpInfo = (HashMap<Integer, ItemStackInfo>) SeCon.getAPI().getObjectUtils().deserializeFromString(serialized);
			...
			SeCon.getAPI().getLogger().severe("Player-Cmd-Collection", "Cannot load virtual chest! name:" + playerName + " error:" + e.getMessage());
			e.printStackTrace();
	...
	public String loadFromDb(String playerName) {
		try {
			DatabaseResult result = SeCon.getAPI().getDatabaseUtils().getDefault().readEnhanced("SELECT * FROM " + getTableName() + " WHERE name = ?", playerName);
			if (result != null && result.hasRows())
				...
				SeCon.getAPI().getDatabaseUtils().getDefault().write("INSERT INTO " + getTableName() + "(`name`, `data`) VALUES (?, ?);", playerName, "");
		} catch (DatabaseException e) {
			e.printStackTrace();
		}
		return null;
	...
	public void save(String playerName) {
		...
		HashMap<Integer, ItemStackInfo> tmpInfo = new HashMap<Integer, ItemStackInfo>();
		for (int i = 0; i < 54; i++) {
		...
		try {
			saveToDb(playerName, SeCon.getAPI().getObjectUtils().serializeToString(tmpInfo));
		} catch (Exception e) {
			SeCon.getAPI().getLogger().severe("Player-Cmd-Collection", "Cannot save virtual chest! name:" + playerName + " error:" + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	private void saveToDb(String playerName, String data) {
		try {
			SeCon.getAPI().getDatabaseUtils().getDefault().write("UPDATE " + getTableName() + " SET `data` = ? WHERE `name` = '" + playerName + "';", data);
		} catch (DatabaseException e) {
			SeCon.getAPI().getLogger().severe("Player-Cmd-Collection", "Cannot save virtual chest! name:" + playerName + " error:" + e.getMessage());
			e.printStackTrace();
		}
